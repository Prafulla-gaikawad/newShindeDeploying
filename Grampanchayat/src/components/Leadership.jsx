...
import dummyImage from '../images/dummy.jpg'; // Fallback image
import sarpanchImage from '../images/sarpanchShinde-सौ शिल्पा अतुल ठोंबरे.jpg';
import adhikariImage from '../images/adhikari-प्रकाश मिलू सुर्यवंशी.jpg';
...
const Leadership = () => {
  ...
  // Use local images for sarpanch and adhikari
  const sarpanchImageUrl = sarpanchImage;
  const sarpanchName = 'सौ. शिल्पा अतुल ठोंबरे';
  const sarpanchRole = 'सरपंच';
  
  // Find adhikari in team members or use default
  const adhikariMember = teamMembers.find(m => 
    m.role?.[language]?.includes('अधिकारी') || 
    m.role?.[language]?.includes('Adhikari') ||
    m.role?.mr?.includes('अधिकारी')
  );
  const adhikariImageUrl = adhikariImage;
  const adhikariName = 'प्रकाश मिलू सुर्यवंशी';
  const adhikariRole = 'ग्राम पंचायत अधिकारी';
  
  // Filter out adhikari from team members if exists, and get other members
  const otherTeamMembers = teamMembers.filter(m => 
    !(m.role?.[language]?.includes('अधिकारी') || 
      m.role?.[language]?.includes('Adhikari') ||
      m.role?.mr?.includes('अधिकारी'))
  );
        ...
        <div className="max-w-6xl mx-auto">
          ...
          {loading ? (
            ...
            <>
              ...
              {/* Team Members Section - Adhikari and Other Members */}
              <div className="mt-24 md:mt-32">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
                  ...
                  {otherTeamMembers.length > 0 ? (
                    otherTeamMembers.slice(0, 2).map((member) => (
                      <div 
                        key={member.id}
                        className="bg-white rounded-lg shadow-lg overflow-hidden border border-gray-100 hover:shadow-xl transition-all duration-300 hover:-translate-y-2"
                      >
                    ...
                    // Show placeholder cards when no other team members
                    [1, 2].map((index) => (
                      <div 
